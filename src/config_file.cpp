...
config_file::~config_file() {
    for(STR_MAP_ITER it = _map.begin(); it != _map.begin(); it++){
        delete it->second;
    }
}
...
unsigned int config_file::GetNumber(const std::string &section, const std::string &key, unsigned int default_value) {
    STR_MAP_ITER  it = _map.find(section);
    if(it != _map.end()){
        std::map<std::string, std::string>::const_iterator  it1 = it->second->find(key);
        if(it1 != it->second->end()){
...
// private
unsigned int config_file::parseNumber(const std::string &s) {
